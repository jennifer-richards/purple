<template>
  <div class="flex flex-row items-center">
    ...
    <ComboboxRoot v-model="selectedAuthor" class="relative" :id="props.id">
      <ComboboxAnchor
      ...
      >
        <span v-if="selectedAuthor" class="flex flex-row bg-gray-100 text-xs rounded-lg px-2 py-0.5 shadow-md border-1 border-gray-500 items-center gap-1 px-1">
          ...
          <button type="button" class="rounded-lg bg-gray-200 focus:bg-gray-300 hover:bg-gray-300 p-0.5 text-black border-none" @click="handleClearSelectedAuthor" :label="`Unselect author ${selectedAuthor.name} #${selectedAuthor.personId}`">&times;</button>
        </span>
        <ComboboxInput v-model="inputRef" :disabled="props.disabled" class="outline-none text-sm py-1 border-none h-full placeholder-gray-400" :placeholder=" selectedAuthor?.personId ? `Change author...` : 'Search author'" />
      </ComboboxAnchor>
